...
export async function getProductDetail(productId: number, reqUserId: number) {
  ...
  const tagNameArr: string[] = tagName.map((data) => data.name);
  const addressName: { si: string, gu: string, dong: string } = await addressModel.getAddressNameByUserId(product.user_id);
  const user = await userModel.getUserById(product.user_id);

  delete product.created_at;
  delete product.user_id;

  const likedCount: number = await productModel.getLikedCountByProductId(productId);
  const reqUserLiked = reqUserId === -1 ? 0 : await productModel.hasLiked(productId, reqUserId);
